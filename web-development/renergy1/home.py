...
def app():
    st.title="climate changes and renewable energy in Palestine"
    ...
    def energy_suggestions(city):
        suggestions = ""

        if city in ["Jericho", "Hebron", "Gaza"]:
            suggestions += f"### {city} has high temperature levels\n"
            suggestions += "- High energy consumption (for air conditioning and cooling systems).\n"
            suggestions += "- High energy consumption → Higher pressure on electric grids (leading to higher costs and possible energy shortages).\n"
            suggestions += "- **Solution:** More heat → More sunlight → Use solar energy to generate electricity via solar panels.\n"
        ...
        if city in ["Nablus", "Jenin", "Tulkarm"]:
            ...
            suggestions += "- Wind farms play a major role in wind energy production.\n"
        ...
        if city in ["Jericho", "Gaza", "Ramallah"]:
            ...
            suggestions += "- Therefore, it's recommended to store this solar energy to prevent it from going to waste.\n"
            suggestions += "- **Suggested solutions:**\n"
            suggestions += "  - Build large-scale solar farms to maximize energy production.\n"
            suggestions += "  - Implement battery storage systems to store excess solar energy for nighttime use.\n"

        return suggestions
    city_data = unique_locations[unique_locations["City"] == city_choice].iloc[0]
    st.markdown(energy_suggestions(city_choice))
